...
export const MoleculeCanvas = () => {
  const { selectedMolecule, isAutoRotate, explodeRadius, renderMode } = useExplorerStore();
  ...
  const bondsToRender = useMemo(() => {
    ...
    return selectedMolecule.bonds.map((bond, idx) => {
    ...
    }).filter(b => b !== null) as { id: string; startPos: [number, number, number]; endPos: [number, number, number]; type: number; }[];
        ...
        <group>
          <Center>
            <group>
              ...
              {/* Render Bonds */}
              {renderMode === 'ball-and-stick' && bondsToRender.map((bond) => (
                <BondLink
                  key={bond.id}
