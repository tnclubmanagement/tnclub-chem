...
const createProduct = (formula: string, state: 'solid' | 'aqueous' | 'liquid' | 'gas'): Chemical => {
  return {
    ...
    applicationEn: 'Product of chemical reaction.'
  };
};
...
export const ChemEngine = {
  predictReaction: (reactants: Chemical[]): Reaction | null => {
    ...
    // 1. Alkali Metal + Water (e.g. Na + H2O)
    const isWater = (c: Chemical) => c.formula === 'H₂O' || c.formula === 'H2O';
    const isAlkaliMetal = (c: Chemical) => METAL_REACTIVITY_SERIES.slice(0, 3).includes(c.formula);
    ...
    if ((isAlkaliMetal(c1) && isWater(c2)) || (isAlkaliMetal(c2) && isWater(c1))) {
      ...
      const metalNameEn = metal.nameEn || metal.name;
      ...
      return {
        ...
        descriptionEn: `${metalNameEn} reacts vigorously with water producing an alkaline solution and releasing hydrogen gas.`
      };
    }
    ...
    if (isWater(c1) || isWater(c2)) {
      ...
      if (['salt', 'acid', 'base'].includes(substance.group)) {
        ...
        const actionEn = substance.state === 'solid' ? 'Dissolving' : 'Diluting';
        const subNameEn = substance.nameEn || substance.name;
        ...
        return {
          ...
          reactants: [c1, c2],
          products: [newProduct],
          equationHTML: `${substance.formula} + H₂O &rarr; Solution ${substance.formula}`,
          effect: 'none',
          description: `${action} ${substance.name} bằng nước cất.`,
          descriptionEn: `${actionEn} ${subNameEn} with distilled water.`
        };
      ...
      if (isReactiveMetal(metal)) {
        ...
        if (ionAcid && ionAcid.anion) {
          ...
          return {
            ...
            descriptionEn: `${metalNameEn} dissolves in acid releasing hydrogen gas.`
          };
        }
      }
    }
    ...
    if (isAcidBase || hasPrecipitate) {
      ...
      return {
        ...
        descriptionEn: isAcidBase ? 'Neutralization reaction forming salt and water.' : 'Precipitation reaction.'
      };
    }
